...
% sets
function extract_sift(type, kp_or_dense, step_size)
    ...
    extract_features(type, kp_or_dense, step_size, './Caltech4/ImageSets/train.txt');
    extract_features(type, kp_or_dense, step_size, './Caltech4/ImageSets/test.txt');
end

function extract_features(type, kp_or_dense, step_size, file)
    for tline=file_list(file)
        ...
        if strcmp(kp_or_dense, 'dense') == 1
            [~, ~, ~] = mkdir(strcat(root_folder, type, '/', kp_or_dense), int2str(step_size));
        end
        
        output_file = descriptor_file_name(folder, type, kp_or_dense, step_size, file_name);
        if exist(output_file{1}, 'file') == 0
            output_file{1}
            features = feature_extraction(strcat('./Caltech4/ImageData/', tline{1}, '.jpg'), type, kp_or_dense, step_size);
